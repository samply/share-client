...
public class CheckScheduledInquiriesJob implements Job {
  ...
  private static final Logger logger = LoggerFactory.getLogger(CheckScheduledInquiriesJob.class);
  ...
  /**
   * check the schedule time of processing inquiries and send a error message to the broker if the
   * inquiry took too much time to be executed.
   *
   * @throws JobExecutionException while abandoned InquiryDetails
   */
    ...
    try {
      ...
      for (InquiryDetails inquiryDetails : inquiryDetailsList) {
        if (checkScheduledAt(inquiryDetails)) {
          ...
          stringBuilder.append("Inquiry with ID ").append(inquiry.getId())
              .append("was abandoned because the execution time was too long. ");
  ...
  private void abandonedInquiryDetails(InquiryDetails inquiryDetails) throws SchedulerException {
    InquiryDetails inquiryDetailsTmp = InquiryDetailsUtil
        .fetchInquiryDetailsById(inquiryDetails.getId());
  ...
  private void unscheduleJob(int inquiryId) throws SchedulerException {
    for (JobExecutionContext jobExecutionContext : ApplicationBean.getScheduler()
        .getCurrentlyExecutingJobs()) {
      if (jobExecutionContext.getMergedJobDataMap().get(ExecuteInquiryJobParams.INQUIRY_ID)
          .equals(inquiryId)) {
  ...
  private boolean checkScheduledAt(InquiryDetails inquiryDetails) {
    int minutes = ConfigurationUtil.getConfigurationTimingsElement(
        EnumConfigurationTimings.JOB_CHECK_SCHEDULED_INQUIRY_INTERVAL_MINUTES).getSetting();
