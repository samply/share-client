...
public class DirectorySyncJob implements Job {
    ...
    private DirectoryApi createDirectoryApi() throws CredentialNotFoundException, IOException {
        List<Credentials> credentialsList = CredentialsUtil.getCredentialsByTarget(TargetType.TT_DIRECTORY);
        ...
        return DirectoryApi.createWithLogin(HttpClients.createDefault(), ApplicationBean.getUrlsForDirectory().getDirecotryUrl(), credentials.getUsername(), credentials.getPasscode());
    }

    private FhirApi createFhirApi() {
        IGenericClient client = ctx.newRestfulGenericClient(ApplicationBean.getUrlsForDirectory().getLdmUrl());
