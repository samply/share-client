...
public class MdrUtils {
     ...
     *
     * @param theList
     *            the list to extend with new dataelements
     * @param groupKey
     *            the mdr id of the dataelementgroup to add
     * @return the elements from the group and its subgroups
     */
    private static ArrayList<MdrIdDatatype> getElementsFromGroupAndSubgroups(ArrayList<MdrIdDatatype> theList, String groupKey) throws MdrConnectionException, ExecutionException {
        List<Result> result_l = ApplicationBean.getMdrClient().getMembers(groupKey, "de");
     ...
     * @return a ViewFields object with either just one (dummy) entry or the complete set of dataelements from mds-b/k
     */
    public static ViewFields getViewFields(boolean completeMds) throws MdrConnectionException, ExecutionException {
        ArrayList<MdrIdDatatype> mdsk_attributes = new ArrayList<>();
        ArrayList<MdrIdDatatype> mdsb_attributes = new ArrayList<>();
        ViewFields viewFields = new ViewFields();

        if (completeMds) {
            mdsk_attributes = getElementsFromGroupAndSubgroups(mdsk_attributes, ConfigurationUtil.getConfigurationElementValue(MDR_GRP_MDSK));
            mdsb_attributes = getElementsFromGroupAndSubgroups(mdsb_attributes, ConfigurationUtil.getConfigurationElementValue(MDR_GRP_MDSB));
            for (MdrIdDatatype attribute : mdsk_attributes) {
                viewFields.getMdrKey().add(attribute.getLatestCentraxx());
            }
            for (MdrIdDatatype attribute : mdsb_attributes) {
                viewFields.getMdrKey().add(attribute.getLatestCentraxx());
            }
        } else {
     ...
     */
    public static Map<String, String> getValuesAndDesignations(String mdrKey, String languageCode, boolean includeIdentical) {
        ...
        try {
            ...
            if (dataType.equalsIgnoreCase(VALIDATION_DATATYPE_ENUMERATED)) {
                ...
                for (PermissibleValue pv : permissibleValues) {
                    ...
                    for (Meaning m : meanings) {
                        if (m.getLanguage().equalsIgnoreCase(languageCode)) {
                            // Add it to the map if either the value differs from the designation or the include switch is true
