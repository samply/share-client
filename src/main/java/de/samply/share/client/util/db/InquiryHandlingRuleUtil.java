...
public class InquiryHandlingRuleUtil {
    ...
    public static int insertInquiryHandlingRule(InquiryHandlingRule inquiryHandlingRule) {
        ...
        InquiryHandlingRuleRecord inquiryHandlingRuleRecord = dslContext.newRecord(Tables.INQUIRY_HANDLING_RULE, inquiryHandlingRule);
    ...
    /**
     * Check if the amount of patients shall be replied
     *
     * Checks inquiry handling rules for the broker it was received from
     *
     * @param inquiry the inquiry for which to check
     * @return if the amount of patients shall be replied
     */
    public static boolean replyCountForInquiry(Inquiry inquiry) {
        try {
            // TODO: when there are more inquiry handling rules, a more sophisticated approach is needed
            for (InquiryHandlingRule inquiryHandlingRule : InquiryHandlingRuleUtil.fetchInquiryHandlingRulesForBrokerId(inquiry.getBrokerId())) {
    ...
    /**
     * Check if the patient data shall be replied
     *
     * Checks inquiry handling rules for the broker it was received from
     *
     * @param inquiry the inquiry for which to check
     * @return if the patient data shall be replied
     */
    public static boolean replyDataForInquiry(Inquiry inquiry) {
        try {
            // TODO: when there are more inquiry handling rules, a more sophisticated approach is needed
            for (InquiryHandlingRule inquiryHandlingRule : InquiryHandlingRuleUtil.fetchInquiryHandlingRulesForBrokerId(inquiry.getBrokerId())) {
