...
public class LdmPostQueryParameterView extends AbstractLdmPostQueryParameter {
   ...
   *
   * @param statisticsOnly              if true, set a parameter to only request a count of the
   *                                    results, not the whole result lists
   * @param removeKeysFromView          A list of keys to be removed from the query (and
   *                                    viewfields)
   * @param completeMdsViewFields       if true, add all entries from mds-b and mds-k to viewfields
   * @param includeAdditionalViewfields if true, check if there are additional viewfields to set in
   *                                    the database. For uploads to central mds database, this
   *                                    should be false
   */
  public LdmPostQueryParameterView(boolean statisticsOnly, List<String> removeKeysFromView,
