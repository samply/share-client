...
public class CredentialsUtil {
    ...
    /**
     * Update the proxy credentials in the database according to those read from a samply common config file
     *
    ...
    public static void updateProxyCredentials(Configuration configuration) {
        ...
        if (configuration.getProxy().getHTTP() != null && configuration.getProxy().getHTTP().getUrl() != null && configuration.getProxy().getHTTP().getUrl().toString().length() > 0) {
            ...
            logger.info("Delete HTTP Proxy Credentials from DB since no HTTP proxy is configured any more");
            deleteCredentials(httpProxyCredentials);
        }
        changed = false;
        Credentials httpsProxyCredentials = getCredentialsForHttpsProxy();
        if (configuration.getProxy().getHTTPS() != null && configuration.getProxy().getHTTPS().getUrl() != null && configuration.getProxy().getHTTPS().getUrl().toString().length() > 0) {
            ...
            logger.info("Delete HTTPS Proxy Credentials from DB since no HTTPS proxy is configured any more");
        ...
        try {
            for (Credentials credentials : fetchCredentials()) {
                if (credentials.getTarget() == TargetType.TT_LDM) {
                    return "Basic " + DatatypeConverter.printBase64Binary((credentials.getUsername() + ":" + credentials.getPasscode()).getBytes("UTF-8"));
                }
            }
